...
import { findEmployee, findEmployeesByManager } from './data.js';
...
export function deriveRole(principal: Principal): Role {
  ...
  const emp = resolveUserEmail(principal) ? findEmployee(resolveUserEmail(principal)!) : undefined;
  return (emp?.role as Role) ?? 'employee';
}
